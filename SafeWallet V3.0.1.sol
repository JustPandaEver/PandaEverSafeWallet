...
contract PandaEverFactory{
    ...
    uint256 public WDFee;
    uint256 public WDTokenFee;
    address[] public allWallets;
    uint256 public chainId = block.chainid;
    event WalletCreated(address UserWallet, uint);

    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'PandaEver: FORBIDDEN');
        feeTo = _feeTo;
    }

    function setWdFee(uint256 _wdfee) external {
        require(msg.sender == feeToSetter, 'PandaEver: FORBIDDEN');
        WDFee = _wdfee;
    }

    function setWdTokenFee(uint256 _wdTokenfee) external {
        require(msg.sender == feeToSetter, 'PandaEver: FORBIDDEN');
        WDTokenFee = _wdTokenfee;
    }
    ...
    function createWallet(address Signersadd, address user) external returns (address UserWallet) {
        require(user != address(0), 'PandaEver: ZERO_ADDRESS');
        require(Signersadd != address(0), 'PandaEver: ZERO_ADDRESS');
        bytes memory bytecode = type(PandaEverSafeWallet).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(Signersadd, user, allWallets.length));
        assembly {
            UserWallet := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IERC20(UserWallet).initialize(Signersadd, user, allWallets.length);
    ...
    function withdrawtoken(address smartcontract, uint256 _amount, address receiver, bytes memory sig, uint256 time) public nonReentrant {
        ...
        if(smartcontract == PEFactory.FeeToken()){
            ...
            if(PEFactory.FeeToken() == address(0)){
                ...
                require(IERC20(PEFactory.FeeToken()).balanceOf(address(this)) > tfee, "Fee Required Not enough");
